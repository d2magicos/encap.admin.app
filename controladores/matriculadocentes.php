...
{
require_once "../modelos/Matriculadocentes.php";
$compra=new MatriculadoDocentes();
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		// 	 Generar codigo matricula
		 ...
		 $rspta=$compra->buscarCategoria('9',$fecha_hora);
		 ...
		 if( $result<=0){
		   ...
		   if (empty($idmatricula)){		
			   $rspta=$compra->insertar($idmatricula,$idpersonal,$fecha_hora,$cod_matricula,$idparticipante,$idcurso1,$fecha_inicio1,$qr,'',$horas,$contexto,$imagen,$imagenposterior,$estilo,$categoria);
			 			//			  `idmatricula`, `idpersonal`, `fecha_hora`, `cod_matricula`, `idparticipante`, `idcurso`, `fecha_inicio`, `qr`,  `fechainfo`, `condicion`, `horas`, `contexto`, `nota`, `año`,imagen,imagenposterior,idplantilla,categoria  
	...
	case 'listarArticulos':
		...
		while ($reg=$rspta->fetch_object()) {
            ...
            "3"=>$reg->n_horas,
			"4"=>$reg->fecha_inicio,
            "5"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idcurso.',\''.$reg->cod_curso.'\',\''.$reg->nombre.'\',\''.$reg->tipo_curso.'\',\''.$reg->n_horas.'\',\''.$reg->fecha_inicio.'\',\''.$reg->contexto.'\')"><span class="fa fa-plus"></span></button>'
              );
		}
		$results=array(
