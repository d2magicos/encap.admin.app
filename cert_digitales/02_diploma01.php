<?php
  ...
  $id = $_GET['id'];
  $imprimir = (!empty($_GET['imprimir'])) ? $_GET['imprimir'] : "";

  $conexion->set_charset("utf8");

  $sQuery = mysqli_query($conexion, "SELECT *,
                                      p.nombre AS nombrep,p.num_documento AS numdoc,m.nota AS nota,m.contexto AS context,
  ...
  if ($dCerti == null) {
    require_once('../public/templates/_404/404_02.php');
    return false;
  }

  if ($dCerti["certificado"] === "SI") {
    ...
    if (empty($dCerti["imagen"])) {
      $sQuery2 = mysqli_query($conexion, "SELECT *,
                                            c1.imagen as imagen,c1.imagenposterior as imagenposterior,
                                            c1.imagenf as imagenf,c1.imagenposteriorf as imagenposteriorf 
    ...
    if ($id == '11039-80421202' && $dCerti["cod_matricula"] == "11039-80421202") {
        $adicional = "BÁSICO - INTERMEDIO - AVANZADO";
        
        $pdf->setY(130);
        $pdf->setX(80);
        
        $pdf->SetFont('Arial', 'B', 22);  
        $pdf->MultiCell(160, 7, utf8_decode($adicional), 0, 'C');
    }
    ...
    if ($str >= 20 and $str <= 32 and $dCerti["cod_matricula"] != "11039-80421202") {
    ...
    if ($str >= 81 and $str <= 95) {
      ...
      //    $pdf->MultiCell(200, 15, utf8_decode($curso), 1, 'C');
      $pdf->MultiCell(185, 13, utf8_decode($curso), 0, 'C');
    };
