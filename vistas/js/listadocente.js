...
function init(){
	...
	$.post("../controladores/listadocentes.php?op=selectMediospagos", function(r){
		$("#idmediospagosm").html(r);
		$('#idmediospagosm').selectpicker('refresh');
	});

	//Cargamos los items al select Forma de recaudacion
	$.post("../controladores/listadocentes.php?op=selectRecaudacion", function(r){
...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
	   ...
	   // "scrollY":"800px",//tamaño del scroll
		...
		"ajax":
				{
					url: '../controladores/listadocentes.php?op=listar',
					type : "get",
					dataType : "json",						
					error: function(e){
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../controladores/listadocentes.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(idmatricula)
{
	...
	$.post("../controladores/listadocentes.php?op=mostrar",{idmatricula : idmatricula}, function(data, status)
	{
		...
		$.post("../controladores/listadocentes.php?op=selectSub&id="+idcurso, function(r){
	...
	//enviar mediante get listar detalle a la varible op de ajax
 	$.post("../controladores/listadocentes.php?op=listarDetalle&id="+idmatricula,function(r){
	        $("#detallesm").html(r);
	});
...
function enviar(idmatricula)
{
	swal({
			...
			showLoaderOnConfirm: true
					...
					if (isConfirm){
						$.post("../controladores/listadocentes.php?op=enviar", {idmatricula : idmatricula}, function(e){
							swal(
								'!!! Entregado !!!',e,'success')
					        tabla.ajax.reload();
				        });
					}else {
						$.post("../controladores/listadocentes.php?op=noenviar", {idmatricula : idmatricula}, function(e){
							swal(
...
function habilitarCert(idmatricula)
{
	swal({
			...
			showLoaderOnConfirm: true
					...
					if (isConfirm){
						$.post("../controladores/listadocentes.php?op=habilitar", {idmatricula : idmatricula}, function(e){
							swal(
								'Habilitado',e,'success')
					        tabla.ajax.reload();
				        });
					}else {
						$.post("../controladores/listadocentes.php?op=nohabilitar", {idmatricula : idmatricula}, function(e){
							swal(
...
function EliminarRegistro(idmatricula){
	swal({
		...
		showLoaderOnConfirm: true
				...
				if (isConfirm){
					$.post("../controladores/listadocentes.php?op=eliminar", {idmatricula : idmatricula}, function(e){
						swal(
							'Registro eliminado',e,'success')
						tabla.ajax.reload();
					});
				}else{
					$.post("../controladores/listadocentes.php?op=nohabilitar999", {idmatricula : idmatricula}, function(e){
						swal(
