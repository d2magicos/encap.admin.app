...
$('.btn_new_cliente').click(function(e) {
       ...
       $.post("../controladores/matriculadocentes.php?op=selectPais", function(r){
           $("#idpais").html(r);
   ...
   //Cargamos id
   $.post("../controladores/matriculadocentes.php?op=id", function($id){
   ...
   $.post("../controladores/matriculadocentes.php?op=idmatriculaprimary", function($id){
       //$("#cod_matricula").html(r);
       $('#idmatricula').val(($id));			
...
$('#btnBuscar').click(function(e) {
   ...
   if(num_digitos<5){
       swal({
           ...
           text: 'Debe ingresar mínimo 8 digitos.',
           html:
             '' +
             'and other'
         });
   ...
   $.ajax({
     url: '../controladores/matriculadocentes.php?op=buscarcliente',
     type: "POST",
     async: true,
     ...
     success: function(response) {
       ...
       if (response == 0) {
           ...
           swal({
               icon: 'error',
               type: 'error',
               title: 'Docente no encontrado <br><br> <a href="docentes.php" class="btn btn-success tepull-center" style="width: 270px; height:40px; font-size:20px;"><i class="fa fa-user-plus"></i> Crear nuevo Docente <br></a> ',
         ...
         $('.btn_new_cliente').slideDown();
           ...
           swal("¡Docente Encontrado!", "", "success");
...
// Crear cliente = Matricula
 $('.btn_cliente').click(function(e) {
   e.preventDefault();
   $.ajax({
     url: '../controladores/matriculadocentes.php?op=nuevocliente',
     type: "POST",
     async: true,
...
function init(){
   ...
   //Cargamos los items al select tipodocumento
       $.post("../controladores/matriculadocentes.php?op=selectTipodocumento", function(r){
   ...
   $.post("../controladores/matriculadocentes.php?op=selectMediospagos", function(r){
       $("#idmediospagos").html(r);
       $('#idmediospagos').selectpicker('refresh');
   });

   //Cargamos los items al select Procedimiento
   $.post("../controladores/matriculadocentes.php?op=selectRecaudacion", function(r){
       $("#idforma_recaudacion").html(r);
       $('#idforma_recaudacion').selectpicker('refresh');
   });

   //Cargamos los items al select Procedimiento
   $.post("../controladores/matriculadocentes.php?op=selectTrafico", function(r){
       $("#idtrafico").html(r);
       $('#idtrafico').selectpicker('refresh');
   });
...
function listar($idpersonal)
{
   tabla=$('#tbllistado').dataTable(
   {
       ...
       "ajax":
               {
                   url: '../controladores/matriculadocentes.php?op=listar',
                   type : "get",
                   dataType : "json",						
                   error: function(e){
...
function guardaryeditar(e)
{
   ...
   $.ajax({
       url: "../controladores/matriculadocentes.php?op=guardaryeditar",
       type: "POST",
       data: formData,
       contentType: false,
       processData: false,
       ...
       {                  
           ...
           if(datos=="Debe asignar una plantilla a este curso."){
               swal({
                   title: 'No se registro la matricula',
                   type: 'warning',
                     text:'No se encontro una plantilla predefinida, revise que se haya asignado una plantilla desde la opción Plantillas para la categoria docentes.'
                 });
           }else{
               swal({
...
function id($id)
{
   ...
   $.post("../controladores/matriculadocentes.php?op=id",{$id : $id}, function(data, status)
   {
...
function idmatriculaprimary($id)
{
   ...
   $.post("../controladores/matriculadocentes.php?op=idmatriculaprimary",{$id : $id}, function(data, status)
   {
       data = JSON.parse(data);		
...
function listarArticulos(){
   ...
   tabla=$('#tblarticulos').dataTable({
       ...
       "ajax":
       {
           url:'../controladores/matriculadocentes.php?op=listarArticulos',
           type: "get",
           dataType : "json",
           error:function(e){
               console.log(e.responseText);
