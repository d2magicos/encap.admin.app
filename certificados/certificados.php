...
<?php
    ...
    if ($doc != null) {
        /* Get student */
        $resultadop = mysqli_query($conexion, "SELECT p.nombre, p.num_documento, p.departamento, p.ciudad, p.email, m.idcurso, c.nombre as curso, m.fecha_inicio as fecha, c.contexto, cat.nombre as categoria, m.cod_matricula, m.idplantilla, m.certificado, m.estadosatisfacion
                                               FROM persona p 
                                               ...
                                               WHERE m.cod_matricula='".$doc."'");
    } else if ($prof != null) {
        $resultadop = mysqli_query($conexion, "SELECT p.nombre, p.num_documento, p.departamento, p.ciudad, p.email, m.cod_matricula, m.idcurso, c.nombre as curso, c.fecha_inicio as fecha, cat.nombre as categoria, c.contexto, m.idplantilla, m.certificado
                                               FROM docente p 
                                               INNER JOIN matricula_docentes m ON m.idparticipante = p.idpersona
                                               INNER JOIN cursos c ON m.idcurso = c.idcurso
                                               INNER JOIN categoria cat ON c.idcategoria = cat.idcategoria
                                               WHERE m.cod_matricula='".$prof."'");
    }
...
<body>
    <header class="header">
        <div class="container header-container">
            <nav class="main-menu">
                ...
                <ul>
                    <li class="actived"><a href="./certificados.php?consultarid=<?= $doc; ?>">Inicio</a></li>
                    <!--<li class="menu-item"><a href="">Ayuda</a></li>-->
                    <li class="menu-item"><a href="../certificados">Salir</a></li>
                </ul>
            </nav>
            <div class="help-container text-center">
                ...
                <a href="https://wa.link/ukukov" target="_blank" title="Click Aquí">
                    <div class="numbers">
        ...
        <main>
            <div class="container my-2">
                <div class="user-container">
                    <div class="user-card">
                        <img class="medalla-image" src="./images/medalla.png" alt="">
                        <div class="header-card">
                            <div class="banner text-center">
                                ...
                                <div class="user-details">
                                    <h2 class="name"><?= $consulta['nombre']; ?></h2>
                                    <div class="">
                                        <span class="user-doc">Documento: <?= $consulta['num_documento']; ?></span><br/>
                                        <span class="user-lugar"><?= $consulta['ciudad']; ?>, <?= $consulta['departamento']; ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="bod-card">
                            <div class="text-center">
                                <h2 class="course-text m-auto"><?= $consulta['curso']; ?></h2>
                                <h4 class="categoria px-4 fw-bold"><?= $consulta['categoria']; ?></h4>
                                <div class="line m-auto my-3"></div>
                                <p class="text-date px-5"><?= $consulta['fecha']; ?></p>
                                <p class="text-context px-5" ><?= $consulta['contexto']; ?></p>
                                <?php
                                    if ($doc != null) {
                                        if ($consulta['certificado'] == "SI") {
                                            /* echo '<a class="button btn-certificado" href="../cert_digitales/'. $consulta['idplantilla'] .'?id='. $consulta['cod_matricula'] .'><i class="fa-solid fa-file-pdf mr-1"></i>Ver certificado</a>'; */
                                            if ($consulta['estadosatisfacion'] == "CONFIRMADO") {
                                                echo '<a class="button btn-certificado" target="_blank" href="../cert_digitales/'. $consulta['idplantilla'] .'?id='. $consulta['cod_matricula'] .'"><i class="fa-solid fa-file-pdf"></i>Ver certificado</a>';;
                                            } else {
                                                echo '<a id="btnEncuesta" onclick="resetButton();" class="button btn-encuesta" codigo="'. $consulta['cod_matricula'] .'" data-bs-toggle="modal" data-bs-target="#encuestaModal"><i class="fa-solid fa-comment"></i>Ver certificado</a>';
                                            }
                                        } else {
                                            echo '<a target="_blank" class="button btn-loading" id="btnCertificadoNull"><i class="fa-solid fa-spinner loading-icon spin"></i>Certificado en Proceso</a>';
                                        }
                                    } else {
                                        echo "";
                                    }
    ...
    <div class="help-mobile-container text-center">
        <a href="https://wa.link/ukukov" target="_blank" title="Click Aquí">
            <div class="help-mobile-button mt-1 mb-4">
                <div class="py-2">
                    <nav class="help-title">Comunícate con el Área de Soporte</nav>
                    <div class="pt-1"><nav class="number-1"><i class="fa-solid fa-phone mx-2"></i>925 248 166</nav></div>
                </div>
